...
[CreateAssetMenu(menuName = "Cards/SwapKingRook")]
public class SwapKingRook : Card
{
    ...
    public override bool CanPlay(Tile tile, Piece piece)
    {
        ...
        if (selected.Count == 0)
        {
            // First selection: allow King or Rook
            return piece is King || piece is Rook;
        }
        else if (selected.Count == 1)
        {
            ...
            if (first is King && piece is Rook) return true;
            if (first is Rook && piece is King) return true;
            return false;
        }
        return false;
    }
